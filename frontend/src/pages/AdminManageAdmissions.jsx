...
const AdminManageAdmissions = () => {
  ...
  const handleUpdateStatus = async (e) => {
    e.preventDefault()
    try {
      const response = await api.put(`/admission/${selectedAdmission.admissionId}/status`, statusForm)
      const result = response.data || {}

      let successText = result.message || 'Admission status updated successfully!'

      // A temporary password is returned only when a NEW Student account is created.
      // This is useful for project/demo admissions that use a fake email address.
      if (result.studentAccountCreated && result.temporaryPassword) {
        successText = `${successText}
Roll Number: ${result.rollNumber}
Login Email: ${result.loginEmail}
Temporary Password: ${result.temporaryPassword}

Copy these credentials now. The temporary password is not stored in plain text.`
      }
      ...
      // Keep generated credentials visible longer so Admin has time to copy them.
      setTimeout(() => setMessage({ type: '', text: '' }), result.temporaryPassword ? 30000 : 5000)
    } catch (error) {
      setMessage({
        type: 'danger',
                    ...
                    <div className="row g-3">
                      ...
                      <div className="col-12 text-end pt-2">
                        {statusForm.status === 'Approved' && (
                          <small className="text-muted me-3 align-middle">
                            Approving automatically creates the Student login account.
                          </small>
                        )}
